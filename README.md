...
> **三種靜態演化測試**：[`手機`](phone.html) · [`茶瓶`](studies.html?object=bottle) · [`交通卡`](studies.html?object=suica)。
> 茶瓶演化為循環培養容器；交通卡演化為開口腕環。各自保留原物件特徵，使用不同的新增／移除結構；不是同一套三翼模板。
> `studies.html?object=bottle&test=1`／`studies.html?object=suica&test=1` 各有 7 項檢查；皆是指定相片人工校準的本地 2.5D 程序概念，尚無通用生成或旋轉驗證。
>
> **最新測試入口：[`phone.html`](phone.html)** — 使用指定手機相片做靜態 2127 形態推演。
> 第二版：保留原手機握座，移除大部分矩形機身，長出三片感知翼及外露光導脊柱。
> 原型／未來滑桿、重播生成過程；`phone.html?test=1` 有 9 項檢查。
> 本地程序生成，平面四角及輪廓針對此相片校準，尚非任意物件自動重建；旋轉影片仍在 `index.html`。
> 目前優先驗證造型，以下旋轉架構屬既有方案，其角度與真實表面的黏附仍待驗證。

> **⚠️ 2026-09-06 方向變更（暫定）**：擷取方式由**電動轉盤**改為**攝影箱 + 正面固定鏡頭**，
> 維持 2.5D，目標唔係 360°，而係「正面 → 斜視」嘅仰角轉場。
> 以下 Part 1 大部分旋轉架構（1.2 資料流、1.4 轉一圈當進度條、1.5 硬體）**暫時擱置**，
> 保留作日後如果重啟旋轉時嘅參考。理由同取捨見 `memory-bank/30-decisions.md`「擷取硬件方向」。
...
- **單次循環**：60 秒
- **形式**：Chrome kiosk（單機）+ **攝影箱／正面固定鏡頭**（2026-09-06 暫定，取代電動轉盤）+ 熱感應印表機
