...
- **Any photo that segments now produces an evolution.** A new photo loaded through the file input starts on `generic()`, a silhouette-driven recipe: it measures smoothed cross-sections of the mask along the object's long axis, keeps both ends of the photo, removes the shell between them, and builds an exo-frame that traces and overshoots the measured outline around a core reduced to half its cross-section, whose surface is the photograph itself. It is not auto-classification: measured here, a strawberry, a toy and a glass of water all fall inside the wide-container proportions, so proportions alone would confidently apply the wrong class. The authored recipes stay one click away. `studies.html?object=banana` is its baseline page.
- `studies.html?object=cup`: a third evolution recipe for the wide-container class (width/height 0.6-1.15), authored against `assets/segmentation/water.jpg`. The drinking rim and foot are kept from the photo, the wall is removed, and a suspended inner vessel on three tempering arms is added. Unlike the two bottle renderers its geometry is derived from the mask's measured half-width, not from one fixture's pixel columns. Recipes are still chosen by hand; each has its own supported proportions and picking the wrong one is rejected with the reason and the recipe that fits.
- Edited masks can feed the existing tea-bottle, steel-flask or wide-container renderer directly. Unsupported geometry keeps the editor available. Editing or replacing the mask hides stale evolution results. Recipe selection also updates the narrative.
...
- **All visitor-facing UI text is Japanese** (`lang="ja"` on every page): titles, navigation, controls, status and refusal messages, the segmentation lab, the calibration panel and the legacy `index.html` overlay. Repo documentation and code comments stay in Chinese, and the legacy `index.html?test=1` check names were deliberately left in Chinese because `memory-bank/` refers to them by name.
- Validation: **139 page self-checks**, plus the real UI workflow in `tests/browser.cjs`. See `tests/README.md` for prerequisites and invocation. Rejections name the failing gate condition (aspect / silhouette touching both frame edges / retained area / too small) rather than one generic line; evidence in `previews/input-tests/reject-reasons.json`. Latest evidence: `previews/input-tests/workflow-results.json`, calibrated phone/card screenshots, mask exports and bottle bridge screenshots.
...
The generic recipe's output quality is bounded by segmentation quality: a photo whose background survives (the strawberry's top patch and contact shadow here) produces a frame that traces that error. Next work should improve segmentation on such photos, and add authored recipes for classes worth authoring, rather than widening claims about the generic one. It renders a house form for an unrecognised object; it does not know what the object is. Keep manual correction and explicit rejection until automated geometry is supported by evidence. Camera capture, printing, exhibition cycling and second-view capture remain unimplemented or unvalidated; do not revive rotation work before the still-image flow is ready.
